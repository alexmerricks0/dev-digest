...
import type { DigestResult } from './claude';
...
export async function sendWeeklyNewsletter(env: Env): Promise<void> {
  ...
  const content = await env.DB.prepare(
    `SELECT date, digest FROM daily_digests WHERE date >= ? ORDER BY date DESC`,
  ).bind(weekAgo).all<{ date: string; digest: string }>();
  ...
  const days = content.results.map((row) => ({
    date: row.date,
    data: JSON.parse(row.digest) as DigestResult,
  }));

  const today = new Date().toISOString().slice(0, 10);
  const subject = `${env.SITE_NAME} — Week of ${today}`;
  const baseHtml = buildWeeklyEmail(days, env);
  const workerHost = env.SITE_URL.replace('://', '://').replace('.lfxai.dev', '-worker.alexmerricks.workers.dev');
...
function buildWeeklyEmail(
  days: Array<{ date: string; data: DigestResult }>,
  env: Env,
): string {
  const daySections = days.map((day) => {
    const stories = day.data.must_read.map((item) => `
      <tr><td style="padding: 0 0 16px;">
        <a href="${esc(item.url)}" style="color: #c9a227; text-decoration: none; font-size: 14px; font-weight: 500;">${esc(item.title)}</a>
        <span style="color: #525252; font-size: 12px;"> [${item.source}]</span>
        <p style="color: #737373; font-size: 13px; margin: 4px 0 0; line-height: 1.5;">${esc(item.summary)}</p>
      </td></tr>
    `).join('');

    const worthKnowing = day.data.worth_knowing.map((item) => `
      <tr><td style="padding: 2px 0;">
        <a href="${esc(item.url)}" style="color: #d4d4d4; text-decoration: none; font-size: 13px;">${esc(item.title)}</a>
        <span style="color: #525252; font-size: 12px;"> — ${esc(item.summary)}</span>
      </td></tr>
    `).join('');
    ...
    return `
      ...
      ${worthKnowing ? `
        <tr><td style="padding: 8px 32px 0;">
          <p style="font-family: monospace; font-size: 10px; color: #525252; letter-spacing: 1px; margin: 0 0 8px;">WORTH KNOWING</p>
          <table width="100%" cellpadding="0" cellspacing="0">${worthKnowing}</table>
        </td></tr>
      ` : ''}
